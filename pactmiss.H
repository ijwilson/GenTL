// @file   Time-stamp: <2005-09-20 22:10:02 nijw>
#ifndef PACT_H__
#define PACT_H__
#include "tnt/tnt.h"
...
public:
  ...
  pact(T &hps, const std::vector<double> &dis, int nprm=10
...
protected:
  ...
  const std::vector<double> &d;         //< ref to distances between sites
...
double pact<T>::calc(double rho)
{
  ...
  if (haps.nmissing()>0) throw std::domain_error("This calc version only works"
					    ...
					    , int minfreq)
{
 ...
 // paths to the last site
	...
	haps.change(order[k],lst,flip(haps.value(order[samp],lst)));
      if (missingvals.size()>=1) {
	for (int s=lst-1;s>=missingvals[0];s--) {
	  for (int x=0;x<k;x++) {
	    double p_stay = exp(-rho*d[s]/k);
				     ...
				     , const double &rho, rng &r, int minfreq)
{
  ...
  for (int k=1;k<n;k++) {
	...
	alpha[0][x]=gammamatch[k]/double(k);
    ...
    for (int s=1;s<sites;s++) {
      ...
      for (int x=0;x<k;x++) {
	alpha[s][x]=p*alpha[s-1][x]+(1-p)*sm;
	if (haps(order[x],s)==haps(order[k],s)) alpha[s][x]*=gammamatch[k];
