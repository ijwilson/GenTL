		       ...
		       , TNT::Array2D<bool> &global,bool resamplepath)
{
  ...
  for (int k=1;k<n;k++) {
	...
	+0.5*thetahat/(double(globalcount[s])+thetahat);
      ...
      // instead of 1 term in li & Stephens we need 6
      double smlocal=0.0;
      double smglobal=0.0;
    ...
    for (int s=sites-2;s>=0;s--) {
      ...
      int xprime=path[k][s+1];// xprime is the path ahead
      for (int x=0;x<k;x++) { // loop throught all the possible haplotypes here
	if (xprime<k) {       // path (ahead) local
...
/**********************************************************************/
/** Function to get the relative localness of each chromosome         */
/**********************************************************************/
