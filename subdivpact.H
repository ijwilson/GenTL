...
#include "pact.h"    
/// product of approximate conditionals for a subdivided population
template <typename T>
class subdivpact  : public pact<T> {
public:
  /** the constructor                                                  */
  subdivpact(T &hps, const std::vector<double> &dis 	    
	    ,const TNT::Array1D<int> &loc, int nprm=10	    
	    ,const unsigned int rsd=1,double theta=-1.0):    
    pac(hps,dis,nprm,rsd,theta),pop(loc),path_(n,sites),global_(n,sites),r(rsd) {   
    npops_=0;    
  ...
  double operator()(double rho, const TNT::Array1D<double>  &,
...
protected:  
  ...
  rng r;  
  subdivpact(); // default constructor -- don't want it used  
...
/** operator for an mcmc analysis                                     */
template<typename T>
double subdivpact::operator()(double rho, const TNT::Array1D<double> &F
			      , const std::pair<int,int> &changeorder) 
{
...
template <typename T>
double subdivpact::calc(double rho, const TNT::Array1D<double> & F, TNT::Array2D<int> &path
...
/**********************************************************************/
/** Functon to get the relative localness of each chromome            */
/**********************************************************************/
template <typename T>
TNT::Array2D<double> subdivpact::localise(double rho
					 , const TNT::Array1D<double> &F
...
/**********************************************************************/
template <typename T>
TNT::Array2D<int>   subdivpact::bestpath(double rho
					 , const TNT::Array1D<double> &F
					 ,std::vector<int> &perm)
{
  ...
  for (int i=0;i<nperm;i++) {
    ...
    permute(&order[0],n,r);
    for (int jj=0;jj<n;jj++) rev[order[jj]]=jj;
 ...
 * (as we go) whether or not each position is in the "local" or global population */
template <typename T>
TNT::Array2D<int> subdivpact::sampleSNP(double &lprob,double rho, const TNT::Array1D<double> & F
				       , TNT::Array1D<int> &pos
				       , TNT::Array1D<double> &d1, TNT::Array2D<int> &path
				       , TNT::Array2D<bool> &global,int minfreq)
{
  ...
  for (int k=1;k<n;k++) {
    ...
    for (int SNP=0;SNP<nSNP;SNP++) {
      ...
      double p_local_norec=exp(-rho*d[position]/scaled_rec_rate);
      double p_global_norec=exp(-rho*d[position]/double(k));         //?!
      double plocalcopy=0.0;  // value if this is the first haplotype from this subpopulation
	...
	if (pop[order[ii]]==location) smlocal+=alpha[position][ii];
	...
	if (global[x][position])
      ...
      p_local_norec=exp(-rho*d2/scaled_rec_rate);
      p_global_norec=exp(-rho*d2/(double(k)));//??!!
      for (int x=0;x<k;x++) {
...
******************************************************************************/
template <typename T>
TNT::Array3D<int> subdivpact::SampleCaseControlSNP(TNT::Array1D<double> &weightings
						  , TNT::Array1D<int> &pos
